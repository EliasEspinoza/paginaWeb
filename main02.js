function calcularepFcn() {
    ...
    // Cálculo del peso del equipo vacío
    pesoequipo = (Vtapas + Vcuerpo) * rhoMaterial;
    document.querySelector('#pesoequipo').value = pesoequipo.toFixed(0);

    // Llamada a la nueva función para calcular el peso total del equipo lleno
    let pesoTotalLleno = calcularPesoLleno(diametro, longitud, rho, rhoMaterial, espesortapareal, espesorcuerporeal);
    document.querySelector('#pesoequipolleno').value = pesoTotalLleno.toFixed(0);

    // Validación del espesor de tapas
    ...
    // Validación del espesor del cuerpo
...
function calcularPesoLleno(diametro, longitud, rho, rhoMaterial, espesortapareal, espesorcuerporeal) {
    let VcuerpoInterno = Math.PI * longitud / 12 * Math.pow(diametro, 2);
    let VtapasInterno = Math.PI / 12 * Math.pow(diametro, 3);

    let VolumenTotalLleno = VcuerpoInterno + VtapasInterno;
    let pesoContenido = VolumenTotalLleno * rho;
    ...
    let pesoTotalLleno = pesoequipo + pesoContenido;

    return pesoTotalLleno;
}